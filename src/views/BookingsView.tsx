...
export default function BookingsView({ bookings }: BookingsViewProps) {
  ...
  const filteredBookings = bookings.filter(booking => {
    if (activeTab === 'transport') return ['flight', 'car', 'transport'].includes(booking.type);
    if (activeTab === 'stay') return booking.type === 'stay';
    if (activeTab === 'activity') return booking.type === 'activity';
    return false;
  });

  return (
    <div className="space-y-4">
      {/* 🔴 修改：Header 區域 (移除「我的憑證」文字，加入分頁按鈕與解鎖鈕) */}
      <div className="flex items-center justify-between gap-3 mb-2">
        ...
        <button 
          ...
          className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors shadow-sm border ${isUnlocked ? 'bg-green-100 text-green-600 border-green-200' : 'bg-white text-gray-400 border-gray-100'}`}
        >
          <FontAwesomeIcon icon={isUnlocked ? faUnlock : faLock} />
        </button>
      </div>
                ...
                'bg-emerald-500'
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-sm">
                      <CategoryIcon type={booking.type} className="text-white"/>
                    </div>
                    <div>
                      <h3 className="font-bold text-lg leading-tight">{booking.title}</h3>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-gray-400">
            <div className="bg-gray-100 p-4 rounded-full mb-3">
              <FontAwesomeIcon icon={
                activeTab === 'transport' ? faCar : 
                activeTab === 'stay' ? faBed : faMapSigns
              } className="text-2xl" />
            </div>
